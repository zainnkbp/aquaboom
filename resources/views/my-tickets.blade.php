                    ...
                    <td class="py-6 px-8 text-right">
                      ...
                      @elseif($tx->status === 'pending')
                        <a href="{{ route('payment.doku.pay', $tx->order_id) }}" 
                           @if($tx->payment_url)
                             onclick="if (typeof loadJokulCheckout === 'function') { event.preventDefault(); window.dispatchEvent(new CustomEvent('hide-chat-assistant')); document.body.classList.add('overflow-hidden'); const ex = document.getElementById('jokul_checkout_modal'); if (ex) ex.remove(); loadJokulCheckout('{{ $tx->payment_url }}'); setTimeout(() => { const iframe = document.querySelector('#jokul_checkout_modal iframe'); if (iframe) { iframe.setAttribute('allow', 'payment'); iframe.setAttribute('scrolling', 'yes'); } }, 50); }"
                           @endif
