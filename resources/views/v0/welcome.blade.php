...
<body class="bg-white text-slate-900 pb-20 md:pb-0"
  x-data="{ isBookingOpen: false, isMobileMenuOpen: false, cursorText: '', activeDate: 'today', customDate: '' }"
  :class="{'hide-native-cursor': !isBookingOpen}">
  <div id="custom-cursor" x-ref="cursor" x-text="cursorText"
    :class="{'hover-active': cursorText !== '', 'hidden': isBookingOpen}"></div>

  <!-- Smart Sticky Header (Foolproof Centered) -->
  <div x-data="{ scrolled: false, lastScroll: 0, hidden: false }" @scroll.window.throttle.16ms="
    ...
    <div
      ...
      <div class="bg-white rounded-full p-1 shadow-sm shrink-0">
        <img src="assets/img/logo aquaboom.jpg" alt="Aquaboom Logo" class="h-10 w-auto rounded-full cursor-pointer" />
      </div>
      <nav class="hidden md:flex items-center gap-10 font-outfit font-bold text-sm text-slate-800 px-4">
        <a href="#tentang-kami" class="hover:text-pink-500 transition-colors">Tentang</a>
        <a href="#wahana" class="hover:text-cyan-500 transition-colors">Wahana</a>
        <a href="#fasilitas" class="hover:text-pink-500 transition-colors">Keunggulan</a>
        <a href="#lokasi" class="hover:text-blue-500 transition-colors">Lokasi</a>
      </nav>
      <button @click="isBookingOpen = true" @mouseenter="cursorText='Pesan!'" @mouseleave="cursorText=''"
    ...
    <header class="absolute top-0 left-0 w-full flex justify-between items-center py-6 px-6 transition-all duration-500"
      :class="!scrolled ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-10 pointer-events-none'">
      <div
        ...
        <img src="assets/img/logo aquaboom.jpg" alt="Aquaboom Logo"
          class="h-10 md:h-12 w-auto rounded-full cursor-pointer bg-white" />
      </div>
      <nav class="hidden md:flex items-center gap-10 font-outfit font-bold text-sm text-white drop-shadow-md">
        <a href="#tentang-kami" class="hover:text-pink-300 transition-colors">Tentang</a>
        <a href="#wahana" class="hover:text-cyan-300 transition-colors">Wahana</a>
        <a href="#fasilitas" class="hover:text-pink-300 transition-colors">Keunggulan</a>
        <a href="#lokasi" class="hover:text-blue-300 transition-colors">Lokasi</a>
      </nav>
      <button @click="isBookingOpen = true" @mouseenter="cursorText='Pesan!'" @mouseleave="cursorText=''"
        class="hidden md:block bg-pink-500 text-white hover:bg-pink-600 px-6 py-2.5 rounded-full font-bold shadow-lg transition-all text-sm tracking-wide font-outfit shrink-0">
        Beli Tiket
  ...
  <div x-show="isMobileMenuOpen" style="display: none" class="fixed inset-0 z-[200] md:hidden">
    <div x-show="isMobileMenuOpen" x-transition.opacity.duration.400ms
    ...
    <div x-show="isMobileMenuOpen" x-transition:enter="transition ease-out duration-300"
      ...
      <nav class="flex flex-col gap-8 font-outfit font-black text-3xl text-white">
        <a href="#tentang-kami" @click="isMobileMenuOpen = false"
          class="hover:text-pink-400 transition-colors">Tentang</a>
        <a href="#wahana" @click="isMobileMenuOpen = false" class="hover:text-cyan-400 transition-colors">Wahana</a>
        <a href="#fasilitas" @click="isMobileMenuOpen = false"
          class="hover:text-pink-400 transition-colors">Keunggulan</a>
        <a href="#lokasi" @click="isMobileMenuOpen = false" class="hover:text-blue-400 transition-colors">Lokasi</a>
      </nav>

      <div class="mt-16 w-full max-w-xs">
        <button @click="isMobileMenuOpen = false; setTimeout(() => isBookingOpen = true, 300)"
  ...
  <section
    ...
    <div class="absolute inset-0 z-0 overflow-hidden">
      <div class="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1582650570395-58079ba5fa63?w=800&q=60">
      </div>
      <!-- Additional gradient to make text pop -->
      <div class="absolute inset-0 bg-gradient-to-t from-[#2563EB] via-transparent to-transparent opacity-80"></div>
    </div>

    <div class="relative z-20 text-center px-4 max-w-5xl mx-auto mt-10 flex flex-col items-center">
      <h1 class="text-6xl md:text-[7rem] font-black text-white tracking-tight mb-6 leading-[1.1] font-outfit">
        Splash into <br class="md:hidden" />
        <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-pink-500">Happiness.</span>
      </h1>
      <p class="text-lg md:text-2xl text-blue-100 font-medium mb-12 max-w-2xl mx-auto leading-relaxed">
        Destinasi wisata air keluarga di jantung kota, menghadirkan sensasi
        ekstrem hingga kolam relaksasi.
      </p>

      <div class="flex flex-col items-center">
        <button @click="isBookingOpen = true" @mouseenter="cursorText='Seru!'" @mouseleave="cursorText=''"
        ...
        @php
            $flashSale = \App\Models\TicketPackage::where('is_active', true)->where('type', 'flash_sale')->latest()->first();
        @endphp
        ...
        <div
          ...
          @click="isBookingOpen = true; $dispatch('set-package', { id: {{ $flashSale->id }} })">
          <div x-data="countdownTimer()" x-init="startTimer()"
            ...
            <div class="flex gap-1.5 text-white font-mono font-bold text-lg">
              <div class="bg-slate-900/40 rounded-lg px-2.5 py-1 shadow-inner border border-white/10 tabular-nums"
                x-text="hours"></div>
          ...
          <div class="flex flex-col sm:pl-2 text-center sm:text-left" @mouseenter="cursorText='Jangan terlewat!'"
            @mouseleave="cursorText=''">
            <span class="text-white/80 text-xs font-bold uppercase tracking-wider mb-0.5">{{ $flashSale->name }}</span>
            <div class="flex items-baseline gap-2 justify-center sm:justify-start">
              <s class="opacity-60 text-sm font-normal text-white">Rp {{ number_format($flashSale->price, 0, ',', '.') }}</s>
              <span class="text-yellow-400 font-black text-3xl tracking-tighter font-outfit">Rp {{ number_format($flashSale->effective_price, 0, ',', '.') }}</span>
            </div>
          </div>
        </div>
        @endif
  ...
  <section id="tentang-kami"
    ...
    <div class="max-w-5xl mx-auto px-6 text-center relative z-10">
      ...
      <h2 class="text-5xl md:text-6xl lg:text-[5.5rem] font-black tracking-tighter leading-[1.1] text-slate-900"
        ...
        <span class="block mb-2 reveal-text delay-200">
          <span class="wave-wrapper">
            ...
            <span class="wave-fill-text block" aria-hidden="true">Nirwana Air</span>
          </span>
        </span>
        <span class="block reveal-text delay-300">Tropis Terbaik</span>
      </h2>
      <p class="mt-12 text-lg md:text-xl text-slate-500 font-medium max-w-2xl mx-auto leading-relaxed">
  ...
  <section id="wahana" class="pt-24 pb-32 bg-slate-50 relative z-40 overflow-hidden">
      ...
      <div
        ...
        @foreach($wahanas as $index => $wahana)
          ...
          <div
            ...
            @mouseenter="cursorText='Cobain!'" @mouseleave="cursorText=''">
            <img src="{{ $wahana->image_url }}" alt="{{ $wahana->name }}" loading="lazy"
  ...
  <section id="fasilitas" class="py-32 bg-[#F5F5F7] relative z-40 overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto px-6 relative z-10">
      <div class="text-center mb-16">
        <h2 class="text-4xl md:text-6xl font-black text-slate-900 mb-6 tracking-tight font-outfit">
          Tingkat
          <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-pink-500">Kemewahan</span>
          Baru
        </h2>
      ...
      <!-- Luxury Bento Grid -->
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 auto-rows-auto md:auto-rows-[340px]">
        <!-- Unik (Span 2 cols) -->
        <div @mouseenter="cursorText='Eksklusif!'" @mouseleave="cursorText=''"
          ...
          <div class="relative z-10 h-full flex flex-col justify-center">
            ...
            <h3 class="text-3xl md:text-5xl font-black text-slate-900 mb-4 font-outfit leading-tight tracking-tight">
        ...
        <div @mouseenter="cursorText='Mudah!'" @mouseleave="cursorText=''"
          ...
          <div class="relative z-10 h-full flex flex-col justify-center md:items-center md:text-center">
            <div
            ...
            <h3 class="text-2xl md:text-4xl font-black text-slate-900 mb-4 font-outfit leading-tight tracking-tight">
              Akses<br />Premium
            </h3>
        ...
        <div @mouseenter="cursorText='Mewah!'" @mouseleave="cursorText=''"
          ...
          <!-- Glass shine effect -->
          <div class="absolute inset-0 bg-gradient-to-tr from-white/10 to-transparent opacity-30"></div>

          <div class="relative z-10 h-full flex flex-col md:flex-row items-center gap-10 md:gap-16">
            <div class="flex-1">
              <div
              ...
              <h3 class="text-3xl md:text-5xl font-black text-white mb-4 font-outfit leading-tight tracking-tight">
                Fasilitas Bintang 4
              </h3>
              <p class="text-slate-300 text-lg font-medium leading-relaxed max-w-2xl">
            ...
            <div class="flex gap-3 md:gap-4 shrink-0 flex-wrap justify-center">
              <div
                class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-5 text-center border border-white/20 shadow-2xl">
                <span class="text-2xl md:text-3xl block mb-2 opacity-90">🛍️</span>
                <span class="text-white font-bold text-xs md:text-sm tracking-wide font-outfit">Mall Access</span>
              </div>
              <div
                class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-5 text-center border border-white/20 shadow-2xl">
                <span class="text-2xl md:text-3xl block mb-2 opacity-90">🚿</span>
                <span class="text-white font-bold text-xs md:text-sm tracking-wide font-outfit">VIP Rooms</span>
              </div>
              <div
                class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-5 text-center border border-white/20 shadow-2xl">
                <span class="text-2xl md:text-3xl block mb-2 opacity-90">🕌</span>
                <span class="text-white font-bold text-xs md:text-sm tracking-wide font-outfit">Musholla</span>
              </div>
              <div
                class="bg-white/10 backdrop-blur-md rounded-2xl p-4 md:p-5 text-center border border-white/20 shadow-2xl">
                <span class="text-2xl md:text-3xl block mb-2 opacity-90">🛖</span>
                <span class="text-white font-bold text-xs md:text-sm tracking-wide font-outfit">Gazebo</span>
              </div>
            </div>
          </div>
        </div>
  ...
  <section class="py-32 bg-white relative z-40 overflow-hidden border-t border-slate-100">
    ...
    <!-- Infinite Marquee -->
    <div class="marquee-container" @mouseenter="cursorText='Keren!'" @mouseleave="cursorText=''">
      <div class="marquee-content gap-6 items-center px-3">
        <!-- Group 1 -->
        <img src="https://aquaboombsb.com/wp-content/uploads/2023/12/V1.jpg" loading="lazy"
          class="h-64 w-80 object-cover rounded-3xl shadow-lg border-4 border-white" />
        <img src="https://aquaboombsb.com/wp-content/uploads/2023/12/V2.jpg" loading="lazy"
          class="h-80 w-72 object-cover rounded-3xl shadow-lg border-4 border-white transform rotate-2" />
        <img src="https://aquaboombsb.com/wp-content/uploads/2023/12/V3.jpg" loading="lazy"
          class="h-64 w-80 object-cover rounded-3xl shadow-lg border-4 border-white" />
        <img src="https://aquaboombsb.com/wp-content/uploads/2023/12/Tiket.png"
          class="h-80 w-72 object-cover rounded-3xl shadow-lg border-4 border-white transform -rotate-2" />
    ...
    <div class="max-w-7xl mx-auto px-6 w-full relative z-10 flex justify-end">
      ...
      <div
        ...
        <div class="flex items-start gap-4 mb-8">
          <div class="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center shrink-0 text-cyan-600">
        ...
        <a href="https://maps.app.goo.gl/example" target="_blank" @mouseenter="cursorText='Go!'"
          @mouseleave="cursorText=''"
  ...
  <footer class="bg-slate-900 text-slate-300 relative pt-24 pb-12 mt-0 z-40 border-t border-slate-800">
    <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-12 relative z-20">
      <!-- Brand & Trust Badge -->
      <div class="col-span-1 md:col-span-5">
        <img src="assets/img/logo aquaboom.jpg" alt="Aquaboom"
          class="h-16 rounded-2xl mb-6 bg-white p-2 shadow-lg w-max" />
        <p class="text-sm leading-relaxed max-w-sm mb-8 text-slate-400 font-medium">
        ...
        <!-- The Trust Badge -->
        <div class="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 w-max backdrop-blur-md">
          <img src="https://jatrahotels.com/logo/JHR-White.png" alt="Jatra Hotels & Resorts"
            class="h-10 w-auto opacity-90" />
          <div class="h-8 w-px bg-white/20"></div>
          <div class="flex flex-col">
            <span class="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-0.5">Managed By</span>
            <span class="text-sm font-baskerville text-white tracking-wide">Jatra Hotels & Resorts</span>
          </div>
        </div>
      </div>
      ...
      <div class="col-span-1 md:col-span-3 md:col-start-7">
        <h4 class="text-white font-black mb-6 uppercase tracking-wider text-sm font-outfit">
          Informasi
        </h4>
        <ul class="space-y-4 text-sm font-medium text-slate-400">
          <li>
            <a href="#" class="hover:text-pink-400 transition-colors flex items-center gap-2"><span
                class="w-1.5 h-1.5 rounded-full bg-pink-500"></span> Cara
              Pembelian</a>
          </li>
          <li>
            <a href="#" class="hover:text-cyan-400 transition-colors flex items-center gap-2"><span
                class="w-1.5 h-1.5 rounded-full bg-cyan-500"></span>
              Peraturan Wahana</a>
          </li>
          <li>
            <a href="#" class="hover:text-orange-400 transition-colors flex items-center gap-2"><span
                class="w-1.5 h-1.5 rounded-full bg-orange-500"></span>
              Syarat & Ketentuan</a>
          </li>
        </ul>
      </div>
    ...
    <div
      ...
      <div class="flex gap-4">
        <a href="#" class="text-slate-500 hover:text-white transition-colors"><svg class="w-5 h-5" fill="currentColor"
            viewBox="0 0 24 24">
            <path
  ...
  <div x-data="socialProof()" x-init="startPopups()"
    class="fixed bottom-24 md:bottom-10 left-6 z-[95] pointer-events-none">
    <div x-show="show" x-transition:enter="toast-enter" x-transition:enter-start="toast-enter-start"
      x-transition:enter-end="toast-enter-end" x-transition:leave="toast-leave"
      x-transition:leave-start="toast-leave-start" x-transition:leave-end="toast-leave-end"
      ...
      style="display: none" @click="isBookingOpen = true">
      <div
      ...
      <div>
        ...
        <p class="text-[13px] text-slate-800 leading-tight font-medium" x-html="message"></p>
        <p class="text-[10px] text-cyan-600 font-bold mt-1.5 flex items-center">
          Verifikasi API Nyata
          <span class="inline-block w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse ml-1.5"></span>
        </p>
      </div>
    </div>
  </div>
  ...
  <div x-show="isBookingOpen" class="fixed inset-0 z-[100] overflow-hidden" style="display: none">
    <div x-show="isBookingOpen" x-transition.opacity.duration.400ms
      class="absolute inset-0 bg-slate-900/60 backdrop-blur-md" @click="isBookingOpen = false"></div>

    <div x-show="isBookingOpen" x-transition:enter="drawer-enter"
      x-transition:enter-start="translate-y-full md:translate-x-full md:translate-y-0"
      x-transition:enter-end="translate-y-0 md:translate-x-0" x-transition:leave="transition ease-in duration-300"
      ...
      <!-- Mobile Drag Handle -->
      <div class="w-full flex justify-center pt-3 pb-1 bg-white md:hidden rounded-t-3xl md:rounded-none">
        <div class="w-12 h-1.5 bg-slate-200 rounded-full"></div>
      </div>


      <!-- LIVEWIRE CHECKOUT COMPONENT -->
      @livewire('checkout-v0')
